         ...
         setCellRenderer("action", new CellRenderer({render: function(cell, value) {
		     cell.innerHTML = "<a onclick=\"sessionSeguimiento(" + getRowId(cell.rowIndex) + ");\" style=\"cursor:pointer\">" +
				...
				"<img src=\"" + image("editar.png") + "\" border=\"0\" alt=\"revisar\" title=\"Correcciones Pendientes\"/>Correccion</a>";
                  cell.innerHTML += "<br><a onclick=\"sessionVisto(" + getRowId(cell.rowIndex) + ");\" style=\"cursor:pointer\">" +
		 		"<img src=\"" + image("basicset/ok.png") + "\" border=\"0\" alt=\"evaluar\" title=\"Visto Bueno\"/>Visto Bueno</a>";
          }}));
...
function sessionSeguimiento(seccion) {
	$.ajax({ 
		...
		{ 
                    if(response=="ok"){
                        document.location.href='revision.lista.php';
                    }else{
...
function sessionVisto(seccion) {
	$.ajax({ 
		...
		{ 
                    if(response=="ok"){
                        document.location.href='../tutor/proyecto.vistobueno.php';
                    }else{
                       alert("Intente De Nuevo");
                    }
		},
