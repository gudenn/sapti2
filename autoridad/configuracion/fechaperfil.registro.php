...
try {
  ...
  /** HEADER */
  $smarty->assign('title','SAPTI - Fechas de Registro de Prfil');
  $smarty->assign('description','Formulario de Cronograma Fechas de Registro de Perfil');
  $smarty->assign('keywords','SAPTI,Cronograma Fechas de Registro de Perfil,Registro');
  ...
  $menuList[]     = array('url'=>URL . Administrador::URL . 'configuracion/'.basename(__FILE__),'name'=>'Cronograma Fechas de Registro de Perfil');
 ...
 $smarty->assign("semestrecod",$semestrecod);
  ...
  $smarty->assign('columnacentro','admin/fechaperfil/columna.centro.registro.fecha.tpl');
  $id = '';
  if (isset($_GET['cronograma_id']) && is_numeric($_GET['cronograma_id']))
    $id = $_GET['cronograma_id'];
  $cronograma = new Fecha_registro($id);
 ...
 //No hay ERROR
  ...
  if($stado==1){
       $_SESSION['estado']=$stado;
          header("Location: fechaperfil.gestion.php");
