<?php
   require('../_start.php');
	...
	$consulta = "SELECT s.codigo as gestion,CONCAT(u.nombre,' ',apellido_paterno,apellido_materno) as nombre ,p.nombre as titulo,v.estado_vigencia as estadop,p.id
        ...
        WHERE u.id=e.usuario_id AND e.id=i.estudiante_id AND i.semestre_id=s.id AND e.id=pe.estudiante_id AND pe.proyecto_id=p.id and p.tipo_proyecto='PR' and p.estado_proyecto='CO' AND p.estado='AC' AND p.id=v.proyecto_id AND v.estado_vigencia='PO' and s.id='".$p."'";
