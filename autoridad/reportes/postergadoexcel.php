<?php
   require('../_start.php');
	...
	$consulta = "SELECT s.codigo as gestion,CONCAT(u.nombre,' ',apellido_paterno,apellido_materno) as nombre ,p.nombre as titulo,v.estado_vigencia as estadop,p.id
        ...
        WHERE u.id=e.usuario_id AND e.id=i.estudiante_id  and d.semestre_id=s.id and i.dicta_id=d.id AND e.id=pe.estudiante_id and i.estado_inscrito='AC' AND pe.proyecto_id=p.id and p.tipo_proyecto='PR' and p.estado_proyecto='IN' AND p.estado='AC' AND p.id=v.proyecto_id AND v.estado_vigencia='PO' and s.id='".$p."'";
	...
	/** Se agrega la libreria PHPExcel */
		...
		$tituloReporte = "Reportes de Proyectos con Postergados";
		...
		while ($fila =mysql_fetch_array($resultado, MYSQL_ASSOC)) {
			$objPHPExcel->setActiveSheetIndex(0)
        		    ...
        		    ->setCellValue('C'.$i,  $fila['gestion'])
                            ->setCellValue('D'.$i, utf8_encode($fila['estadop']));
					$i++;
		}
		...
		header('Content-Disposition: attachment;filename="ReportePostergado.xlsx"');
