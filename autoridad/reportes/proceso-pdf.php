...
$sql = "SELECT u.nombre AS NOMBRE,CONCAT(apellido_paterno,apellido_materno) as APELLIDOS ,s.codigo as SEMESTRE,p.nombre as TITULO,p.estado as ESTADO
   ...
   WHERE u.id=e.usuario_id AND e.id=i.estudiante_id and p.tipo_proyecto='PR' and p.estado_proyecto='CO' and p.estado_proyecto='CO' AND i.semestre_id=s.id AND e.id=pe.estudiante_id AND pe.proyecto_id=p.id and i.estado_inscrito='AC' AND p.estado='AC' and s.id='".$p."'";
