<?php
   require('../_start.php');
	...
	$consulta = "SELECT p.id,s.codigo as gestion,p.nombre as titulo,CONCAT(u.nombre,' ',apellido_paterno,apellido_materno) as nombre ,p.estado as estadop
        ...
        WHERE u.id=e.usuario_id AND e.id=i.estudiante_id and p.estado_proyecto='CO' AND i.semestre_id=s.id and p.tipo_proyecto='PE' AND e.id=pe.estudiante_id AND pe.proyecto_id=p.id AND p.estado='AC' and s.id='".$p."'";
