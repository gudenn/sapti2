...
header("content-disposition: attachment;filename=prorroga.xls");
?>
<HTML LANG="es">
...
$sql = "SELECT u.nombre,s.codigo,CONCAT(apellido_paterno,apellido_materno) as apellidos ,p.nombre as titulo,v.estado_vigencia as estadop,p.id
...
=s.id AND e.id=pe.estudiante_id AND pe.proyecto_id=p.id and p.tipo_proyecto='PE' AND p.estado='AC' AND p.id=v.proyecto_id AND v.estado_vigencia='PR' and s.id='".$p."'";
...
<td>&nbsp;%s</td>
<td>&nbsp;%s</td>
</tr>", $row["nombre"],$row["apellidos"],utf8_decode($row["titulo"]),$row["codigo"],$row["estadop"]);
}
