...
$sql = "SELECT u.nombre AS NOMBRE,CONCAT(apellido_paterno,' ',apellido_materno) as APELLIDOS,p.nombre as TITULO,e.numero_cambio_leve as CL,e.numero_cambio_total AS CT
  ...
  WHERE u.id=e.usuario_id AND e.id=i.estudiante_id  AND e.id=pe.estudiante_id AND pe.proyecto_id=p.id and d.semestre_id=s.id and i.dicta_id=d.id AND p.estado='AC'and p.tipo_proyecto='PR' and p.estado_proyecto='IN' AND c.proyecto_id=p.id and s.id='".$p."'
