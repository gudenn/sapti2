...
WHERE u.id=e.usuario_id AND e.id=i.estudiante_id AND i.semestre_id=s.id and i.estado_inscrito='AC' AND e.id=pe.estudiante_id AND pe.proyecto_id=p.id and p.tipo_proyecto='PR' and p.estado_proyecto='CO' AND p.estado='AC' AND p.id=v.proyecto_id and p.tipo_proyecto='PR' AND v.estado_vigencia='PO' and s.id='".$p."'";
//$sql = $queEmp = $_GET['sql'];
$b=1;
